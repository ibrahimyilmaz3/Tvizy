...
class Product3Adapter : RecyclerView.Adapter<Product3Adapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): Product3Adapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_product, parent, false)
        context = parent.context
        return ViewHolder(v)
    }

    override fun onBindViewHolder(holder: Product3Adapter.ViewHolder, position: Int) {
        with(holder.itemView) {

            ivProduct.setImageDrawable(
                ContextCompat.getDrawable(
                    (context as AppCompatActivity),
                    product[position].third
                )
            )
